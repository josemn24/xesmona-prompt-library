...
describe("validación de los datos estáticos", () => {
  ...
  it("conserva los 61 prompts y sus identificadores únicos", () => {
    expect(libraryData.prompts).toHaveLength(61);
    expect(new Set(libraryData.prompts.map((prompt) => prompt.id)).size).toBe(61);
    expect(new Set(libraryData.prompts.map((prompt) => prompt.slug)).size).toBe(61);
